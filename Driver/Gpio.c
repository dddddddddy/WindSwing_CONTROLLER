...
void Key_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(KEY2_3_GPIO_CLK | KEY4_GPIO_CLK | KEY5_GPIO_CLK , ENABLE);
	...
	NVIC_InitStructure.NVIC_IRQChannel = EXTI9_5_IRQn;                   //PA8设置为外部中断
  ...
  EXTI_InitStructure.EXTI_Line = EXTI_IMR_MR8;
  EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;                    //设置EXTI线路响应中断请求
  EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;                //下降沿响应中断请求
